...
class SecondaryEffectClass:
    def __init__(self, name: str, probability: int, description: str, effect_function: 'function', turn_total: int = -1, probability_vanishing: int = 0,) -> None:
        self.name: str = name
        self.probability: int = max(probability, 0) # percentage
        self.description: str = description
        self.effect_function: function = effect_function
...
def burn(target: 'Pokemon') -> None:
    if target.status == PrimeStatus.NORMAL and target.type != Type.FIRE:
...
def poison(target: 'Pokemon') -> None:
    if target.status == PrimeStatus.NORMAL and target.type != Type.POISON:
...
def paralysis(target: 'Pokemon') -> None:
    if target.status == PrimeStatus.NORMAL and target.type != Type.ELECTRIC:
...
def freeze(target: 'Pokemon') -> None:
    if target.status == PrimeStatus.NORMAL and target.type != Type.ICE:
...
def leech_seed(target: 'Pokemon') -> None:
    if not Type.PLANT in target.types:
